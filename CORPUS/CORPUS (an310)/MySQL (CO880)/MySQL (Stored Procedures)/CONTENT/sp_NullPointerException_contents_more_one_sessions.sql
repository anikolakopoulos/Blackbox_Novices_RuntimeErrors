...
CREATE DEFINER=`whitebox`@`localhost` PROCEDURE `sp_NullPointerException_contents_more_one_sessions`(IN a DATETIME, IN b DATETIME)
BEGIN
SELECT a, b, a1.created_at, a1.uuid, a2.sessions, a1.exception_class,
	a1.exception_message, a1. `Message Type`, a1.content as content, a1.source_history_type, a1.code, a1.`source_files.id`, a1.`m.id`
FROM
(
	SELECT m.created_at, u.uuid, i.exception_class,	i.exception_message,
	if(i.exception_message LIKE "",'IndexOutOfBoundsException('' '')','IndexOutOfBoundsException(other message type)') AS `Message Type`, 
	h.content as `content`, h.source_history_type AS `source_history_type`, i.code AS `code`, source_files.id AS `source_files.id`, m.id AS `m.id`
FROM `blackbox_production`.`master_events` AS m 
	...
	AND i.exception_class LIKE "java.lang.NullPointerException"
	AND i.exception_message LIKE ""
...
(
	...
	FROM `view_NullPointerException_contents` as c
