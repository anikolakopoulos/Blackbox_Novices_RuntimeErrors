...
CREATE DEFINER=`whitebox`@`localhost` PROCEDURE `sp_StringIndexOutOfBoundsException_contents_more_one_sessions`(IN a DATETIME, IN b DATETIME)
BEGIN
...
(
	...
	i.exception_message,
	if(i.exception_message LIKE "String index out of range: -%", 'StringIndexOutOfBoundsException(-)',
	if(i.exception_message = "String index out of range: 0", 'StringIndexOutOfBoundsException(0)',
	if(i.exception_message LIKE "String index out of range: %",	'StringIndexOutOfBoundsException(+)',
	'StringIndexOutOfBoundsException(other message type)'))) AS `Message Type`, 
...
(
	...
	FROM `view_StringIndexOutOfBoundsException_contents` AS c 
