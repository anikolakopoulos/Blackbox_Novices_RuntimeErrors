...
CREATE DEFINER=`whitebox`@`localhost` PROCEDURE `stored_procedure_StringIndexOutOfBoundsException_total_number`(IN a DATETIME, IN b DATETIME)
BEGIN
SELECT a, b, a1.`Message Type`, a1.`User LifeTime`, SUM(a1.`Instances`) AS `Instances` FROM
(
	SELECT u.uuid, if(i.exception_message LIKE "String index out of range: -%", 'StringIndexOutOfBoundsException(-)', if(i.exception_message = "String index out of range: 0", 'StringIndexOutOfBoundsException(0)', if(i.exception_message LIKE "String index out of range: %", 'StringIndexOutOfBoundsException(+)','StringIndexOutOfBoundsException(other message type)'))) AS `Message Type`, 
	...
	AND ( i.exception_message LIKE "String index out of range: -%" OR
	i.exception_message = "String index out of range: 0" OR
	i.exception_message LIKE "String index out of range: %") 
...
(
	...
	AND i0.exception_class LIKE "java.lang.StringIndexOutOfBoundsException"
	AND ( i0.exception_message LIKE "String index out of range: -%" OR
	i0.exception_message = "String index out of range: 0" OR
	i0.exception_message LIKE "String index out of range: %") 
) AS x
